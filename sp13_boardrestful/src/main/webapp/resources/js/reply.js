...
$(function(){
    ...
    $('#replyConfirm').click(function(){
        ...
        var params = $('#replyFrm').serialize(); // replyer, reply 를 params 에 다 담은것
        ...
        $.ajax({
            ...
            data : params,
            url : '/replies/new',
            success : function(result){ // 요청이 성공 했을시
                alert(result);
                // 화면초기화
                $('#reply').val('');
                // 입력 후 목록보기
    ...
    function replyList(){
        $.ajax({
            ...
            success : function(result){
                ...
                replyList.empty();
                for(row of result){ // result : list 값  row 에 하나씩 찍겠다
                    //console.log(row);
                    var tr = $('<tr/>');
    ...
    $('#replyList').on('click','button', function(){
        ...
        // '수정하기' 버튼일 경우
            ...
            $.ajax({
                type :'put',
                url : '../replies/'+ rno,
                data : JSON.stringify(params), //http body 담겨서 보내짐
